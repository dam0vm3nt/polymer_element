define(['dart_sdk',
        './native_import', '../../node_modules/redux/dist/redux', 'exports',
        'polymerize_require/htmlimport!bower_components/polymer/polymer.html',
        'polymerize_require/htmlimport!bower_components/polymer/lib/mixins/mutable-data.html'
    ],
    function (sdk, native_import, redux, exports) {
        ...
        var DartPropertySupportMixin = Polymer.dedupingMixin((base) => class extends base {
            ...
            constructor() {
                super();
                if (base.prototype.hasOwnProperty('new')) {
                    base.prototype['new'].apply(this);
                }
            }
        ...
        function polymerize(type, tag, config, summary, native, templateUrl) {
            ...
            let polymerizeGenerated =
                ...
                class extends DartPropertySupportMixin(type) {
                    constructor() {
                        super();
                        // call original dart type new if exists
        ...
        let defineBehavior = function (behaviorName, dartClass, config) {
            ...
            let baseBehavior = (base) => base;

            // Add mixins too (before !)
            config.behaviors.forEach(function (b) {
                if (typeof b === 'function') {
                    let oldBehavior = baseBehavior;
                    baseBehavior = function (base) {
                        return oldBehavior(b(base));
                    };
                } else {
                    console.warn(
                        "Cannot mixin legacy behaviors inside behaviors " + b);
                }
            });
            ...
            let generatedBehavior = (base) => {
                ...
                let newClass = class extends baseBehavior(base) {
                    constructor() {
                        super();
                        // Call this new and super too
                        //this['new'] && this['new']();
                        if (newClass.prototype.hasOwnProperty('new')) {
                            newClass.prototype['new'].apply(this);
                        }
        ...
        window.Polymer.DartElement = class extends Polymer.Element {
            constructor() {
                super();
                //console.log(`BOTTOM CONSTRUCTOR for ${this.constructor.is}`);
            }
        ...
        // Patch Object to avoid dart JS interop random cast exception (define a DEFAULT static '_check' and 'as' method that will do nothing)
        Object.defineProperty(Object.prototype, '_check', {
            enumerable: false,
            configurable: false,
            value: (x) => x
        });
        ...
        let _postCallbacks = Symbol('_postCallbacks');
        ...
        window.Polymerize = window.Polymerize || {
            ...
            PropertyObserverBehavior: (base) => class PropertyObserverBehavior extends base {
                ...
                _propertiesChanged(data, changedProps, oldData) {
                    ...
                    if (this.onPropertiesChangedPostHook) {
                        this.onPropertiesChangedPostHook(data, changedProps,
                            oldData);
                    }
                }
            },
            ...
            DartCallbacksBehavior: (base) => class DartCallbacksBehavior extends base {
                ...
                constructor() {
                    ...
                    // Call dart constructor
                    // this['new']();

                    this[_callbacks] = {};
                    this[_postCallbacks] = {};
                    //console.log(`JSCONSTRUCTOR CB BEHAVI for ${this.constructor.is}`);

                }


                _executeCallbacks(name, post, ...args) {
                    (this[(!!post) ? _postCallbacks : _callbacks][name] || []).forEach(cb => {
                        cb.apply(this,args);
                    });
                }

                _registerCallback(name, post, cb) {
                    let cbs = this[(!!post) ? _postCallbacks : _callbacks][name];
                    if (!cbs) {
                        cbs = [];
                        this[(!!post) ? _postCallbacks : _callbacks][name] = cbs;
                    }
                    cbs.push(cb);
                }
                ...
                ready() {
                    ...
                    this._executeCallbacks('ready', false);
                    super['ready'] && super.ready();
                    this._executeCallbacks('ready', true);
                ...
                connectedCallback() {
                    ...
                    this._executeCallbacks('connectedCallback', false);
                    super['connectedCallback'] && super.connectedCallback();
                    this._executeCallbacks('connectedCallback', true);
                ...
                disconnectedCallback() {
                    ...
                    this._executeCallbacks('disconnectedCallback', false);
                    super['disconnectedCallback'] && super.disconnectedCallback();
                    this._executeCallbacks('disconnectedCallback', true);
                ...
                attributeChangedCallback(name, old, value) {
                    ...
                    this._executeCallbacks('attributeChangedCallback', false, name, old, value);
                    super['attributeChangedCallback'] && super.attributeChangedCallback(name, old, value);
                    this._executeCallbacks('attributeChangedCallback', true, name, old, value);
                ...
                registerConnectedCallback(callback, post) {
                    this._registerCallback('connectedCallback', post, callback);
                }


                registerDisconnectedCallback(callback, post) {
                    this._registerCallback('disconnectedCallback', post, callback);
                }

                registerAttributeChangedCallback(callback, post) {
                    this._registerCallback('attributeChangedCallback', post, callback);
                }

                registerReadyCallback(callback, post) {
                    this._registerCallback('ready', post, callback);
                }
            },
            Redux: {
